...
class UserProfileApiTest(TestCase):
    ...
    def test_profile_exists(self):
        """Test that a valid request to the user_profile_api returns the correct user profile data."""
        self.client.login(username='testuser', password='testpass')
        
        # Request to the user_profile_api
        response = self.client.get(reverse('user_profile_api', kwargs={'user_id': self.user.id}))
        ...
        self.assertEqual(json_response['member_since'], self.user.date_joined.strftime("%Y-%m-%d"))

    def test_profile_not_found(self):
        """Test that a request for a non-existent user profile returns a 404 status with the appropriate error message."""
        self.client.login(username='testuser', password='testpass')
        
        response = self.client.get(reverse('user_profile_api', kwargs={'user_id': 9999}))  # Non existent user ID
